...
public class JwtTokenService {
    ...
    /**
     * Kong 模式：仅 Redis 会话/黑名单，受众需在允许列表内。
     */
    public Optional<ValidatedToken> sessionValidAny(String token, AuthAudience... allowedAudiences) {
        if (allowedAudiences == null || allowedAudiences.length == 0) {
            return Optional.empty();
        }
        Set<AuthAudience> allowed = new HashSet<>(Arrays.asList(allowedAudiences));
     ...
     */
    public boolean isSessionValid(String token, AuthAudience expectedAudience) {
        return sessionValidAny(token, expectedAudience).isPresent();
    }
